...
function moveToTargetWithStuckDetection(creep, target, r = 1) {
    ...
    var debug = creep.name == "attack2320";
...
var roleAttack = {
    ...
    attack: function (creep) {
            ...
            if (target == undefined)
                if (creep.hits < 0.6 * creep.hitsMax) {
                    ...
                    if (!needHeal || needHeal.length == 0)
                        needHeal = creep.room.find(FIND_MY_CREEPS,
                            { filter: c => c.hits < c.hitsMax });
        ...
        var healNearby = true;
        if (healNearby && target == undefined && creep.getActiveBodyparts(HEAL) > 0) {
            var needHeal = creep.pos.findInRange(FIND_MY_CREEPS, 5,
        ...
        if (target == undefined) {
            target = creep.room.find(FIND_STRUCTURES, {
                filter: object => (
                    object.structureType == STRUCTURE_POWER_BANK && object.hits > 0)
            })[0];
        }
        ...
        if (target == undefined) {
            target = creep.pos.findClosestByPath(FIND_HOSTILE_STRUCTURES, {
                filter: object => object.structureType != STRUCTURE_WALL &&
                   (!object.store || !object.store.getUsedCapacity())
            });
        }
        ...
        if (target == undefined && creep.room.controller && !creep.room.controller.my) {
        ...
        if (!target && creep.getActiveBodyparts(ATTACK) > 0) {
        ...
        if (creep.getActiveBodyparts(ATTACK) == 0 && creep.getActiveBodyparts(HEAL) > 0) {
            //console.log("no target to attack in room ", creep.room.name, " recycling ", creep.name);
            var noOneToHeal = creep.room.find(FIND_CREEPS, { filter: c => _.some(c.body, b => b.type == ATTACK) }).length == 0;
        ...
        if (creep.getActiveBodyparts(ATTACK) > 0) {
            // do not attack powerbanks if not healed
            if (target.structureType == STRUCTURE_POWER_BANK &&
                creep.hits < 0.7 * creep.hitsMax)
                return true;
        ...
        if (creep.getActiveBodyparts(ATTACK) == 0 && target) {
            console.log(target.structureType, " at ", target ? target.pos : "unknown", " has hits ", target ? target.hits : "unknown");
    ...
    healNearbyWhenFarFromTarget(creep, target, nearbyRange, farRange) {
        if (creep.getActiveBodyparts(HEAL) == 0)
        ...
        var distanceToTarget = creep.pos.getRangeTo(target.pos);
        ...
        var needHeal = creep.pos.findInRange(FIND_MY_CREEPS, nearbyRange,
            { filter: c => c.hits < c.hitsMax });
        ...
        var healTarget = needHeal[0];

        if (creep.pos.isNearTo(healTarget))
            return creep.heal(healTarget);

        return creep.rangedHeal(healTarget);
    },
    ...
    run: function (creep) {
       ...
       // if (creep.hits < 0.05 * creep.hitsMax) {
        //    this.healSelfOrAround(creep);
        //    basic.leaveDangerousRoom(creep);
        //    return;
        //}
